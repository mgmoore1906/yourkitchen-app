'use client'
// FILE: app/onboarding/favorites/page.tsx
...
export default function OnboardingFavorites() {
  ...
  const handleFinish = async () => {
    ...
    const res = await fetch('/api/onboarding', {
      ...
      body: JSON.stringify({ user_id: user.id, full_name: onboarding.full_name, address: onboarding.address, household_size: onboarding.household_size, dietary_restrictions: onboarding.dietary_restrictions || [], restaurants: onboarding.restaurants || [], calendar_dates: onboarding.calendar_dates || [], delivery_window_start: onboarding.delivery_window_start, delivery_window_end: onboarding.delivery_window_end, favorites }),
    })
    ...
    try {
      await fetch('/api/send-email', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ to: user.email, type: 'welcome', name: onboarding.full_name, slug: data.slug, kitchen_name: `${onboarding.full_name}'s Kitchen` }),
      })
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FAFAF5', fontFamily: "'DM Sans', sans-serif", padding: '0 0 40px' }}>
      ...
      <div style={{ display: 'flex', gap: 6, padding: '20px 24px 0' }}>
        {[0,1,2,3].map(i => <div key={i} style={{ flex: 1, height: 4, borderRadius: 4, background: '#3D6B4F' }} />)}
      </div>

      <div style={{ padding: '24px 24px 0', maxWidth: 500, margin: '0 auto' }}>
        <p style={{ fontSize: 10, fontWeight: 500, letterSpacing: 3, color: '#3D6B4F', textTransform: 'uppercase', margin: '0 0 8px' }}>Step 4 of 4</p>
        <h1 style={{ fontFamily: "'Lora', serif", fontSize: 26, fontWeight: 500, color: '#1E2620', margin: '0 0 6px', letterSpacing: -0.5 }}>What do you love to eat?</h1>
        <p style={{ fontSize: 14, color: '#6B7066', margin: '0 0 24px', fontWeight: 300 }}>Star your favorites — coordinators will see these when ordering for you.</p>

        {/* Restaurant tabs */}
        <div style={{ display: 'flex', gap: 8, marginBottom: 20, overflowX: 'auto', paddingBottom: 4 }}>
          {restaurants.map((r, i) => (
            <button key={r.id} onClick={() => setActiveTab(i)}
              style={{ background: activeTab === i ? '#3D6B4F' : '#EAF2ED', color: activeTab === i ? '#fff' : '#3D6B4F', border: 'none', borderRadius: 20, padding: '8px 16px', fontSize: 12, fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap', fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 20 }}>
          {menuItems.length === 0 ? (
            <p style={{ fontSize: 14, color: '#6B7066', textAlign: 'center', padding: '20px 0' }}>No menu items for this restaurant yet.</p>
          ) : menuItems.map(item => (
            <button key={item.id} onClick={() => toggleFav(item.id)}
              style={{ background: favorites.includes(item.id) ? '#EAF2ED' : '#fff', border: `2px solid ${favorites.includes(item.id) ? '#3D6B4F' : '#DDE8E0'}`, borderRadius: 14, padding: '14px 16px', cursor: 'pointer', textAlign: 'left', display: 'flex', gap: 12, alignItems: 'flex-start', transition: 'all 0.15s', fontFamily: "'DM Sans', sans-serif" }}>
              <div style={{ width: 20, height: 20, borderRadius: '50%', flexShrink: 0, border: `2px solid ${favorites.includes(item.id) ? '#3D6B4F' : '#DDE8E0'}`, background: favorites.includes(item.id) ? '#3D6B4F' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 12, marginTop: 2 }}>
                {favorites.includes(item.id) ? '★' : ''}
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: "'Lora', serif", fontSize: 14, fontWeight: 600, color: '#1E2620' }}>{item.name}</div>
                <div style={{ fontSize: 12, color: '#6B7066', fontWeight: 300, marginTop: 2 }}>{item.desc}</div>
              </div>
        ...
        {favorites.length > 0 && (
          <div style={{ background: '#EAF2ED', borderRadius: 12, padding: '14px 16px', marginBottom: 20 }}>
            <p style={{ fontSize: 13, color: '#3D6B4F', margin: 0 }}>⭐ <strong>{favorites.length} favorites</strong> saved across your restaurants.</p>
          </div>
        )}

        {error && (
          <div style={{ background: '#FDE8E8', border: '1.5px solid #B94040', borderRadius: 12, padding: '14px 16px', marginBottom: 20 }}>
            <p style={{ fontSize: 13, color: '#B94040', margin: 0 }}>⚠️ {error}</p>
          </div>
        )}

        <button onClick={handleFinish} disabled={loading}
          style={{ width: '100%', padding: '14px', borderRadius: 10, border: 'none', background: loading ? '#6B9E7E' : '#3D6B4F', color: '#fff', fontSize: 14, fontWeight: 500, cursor: loading ? 'default' : 'pointer', fontFamily: "'DM Sans', sans-serif" }}>
