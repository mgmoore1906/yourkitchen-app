...
const TIERS = [
    ...
    sections: [
      {
        label: 'Calendar',
        items: [
          { text: 'Active window', tag: '60 days', check: true },
          { text: 'Recurring or perpetual calendar', check: false },
        ],
      },
      {
        label: 'Restaurants + Meals',
        items: [
          { text: 'Restaurants', tag: '3 max', check: true },
          { text: 'Menu items per restaurant', tag: '4 max', check: true },
          { text: 'Personal drop-off / home cook option', check: false },
        ],
      },
      {
        label: 'Delivery',
        items: [
          { text: 'DoorDash delivery', check: true },
          { text: 'SMS confirmation flow', check: true },
          { text: 'Tracking link to recipient', check: true },
        ],
      },
      {
        label: 'Sharing + Brand',
        items: [
          { text: 'Shareable kitchen link', check: true },
          { text: 'Social share buttons', check: true },
          { text: 'Custom kitchen URL slug', check: false },
    ...
    sections: [
      {
        label: 'Calendar',
        items: [
          { text: 'Active window', tag: 'Unlimited', check: true },
          { text: 'Recurring date slots', check: true },
          { text: 'Perpetual calendar (long-term care)', check: true },
        ],
      },
      {
        label: 'Restaurants + Meals',
        items: [
          { text: 'Restaurants', tag: '10 max', check: true },
          { text: 'Menu items per restaurant', tag: '12 max', check: true },
          { text: 'Personal drop-off / home cook option', check: true },
        ],
      },
      {
        label: 'Delivery',
        items: [
          { text: 'DoorDash delivery', check: true },
          { text: 'SMS confirmation flow', check: true },
          { text: 'Tracking link to recipient + coordinator', check: true },
          { text: 'Coordinator thank-you SMS on delivery', check: true },
        ],
      },
      {
        label: 'Sharing + Brand',
        items: [
          { text: 'Shareable kitchen link', check: true },
          { text: 'Social share buttons', check: true },
          { text: 'Custom kitchen URL slug', check: true },
          { text: 'Priority support', check: true },
          { text: 'Multiple active Kitchens', tag: '3 max', check: true },
        ],
      },
    ],
  },
  {
    key: 'annual',
    badge: 'Early Adopter',
    name: 'Kitchen Care+ Annual',
    price: '$59',
    period: '/ year',
    highlight: 'Best value',
    highlightColor: '#6B9E7E',
    cta: 'Start 14-day free trial',
    ctaStyle: 'primary',
    sections: [
      {
        label: 'Everything in Care+, plus',
        items: [
          { text: 'Save 50% vs monthly billing', check: true },
          { text: 'Early Adopter badge on Kitchen page', check: true },
          { text: 'Locked-in annual rate — never increases', check: true },
          { text: 'Early access to new features (beta)', check: true },
        ],
      },
    ],
  },
  {
    ...
    name: 'Kitchen Care+ Founding',
    price: '$149',
    period: 'lifetime',
    highlight: 'Limited — founding members',
    highlightColor: '#C17F47',
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .plan-footer a { color: var(--sage); text-decoration: none; }
        ...
        @media (max-width: 600px) {
          .onb-nav { padding: 14px 20px; }
          .prog-label { display: none; }
          .plan-grid, .plan-grid-bottom { grid-template-columns: 1fr; }
          .onb-body { padding: 32px 20px 60px; }
        }
      `}</style>
              ...
              <div>
                ...
                <div className="field-wrap">
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <select className="field-input" style={{ width: 80, flexShrink: 0 }}>
                      <option>🇺🇸 +1</option>
                    </select>
            ...
            {step === 'address' && (
              <div>
                ...
                <div className="geo-note">
                  📍 <strong>Launching in major US cities.</strong> We'll confirm restaurant availability at your ZIP — if we're not there yet, we'll notify you the moment we are.
                </div>
              ...
              <div>
                ...
                <div className="plan-grid">
                  {TIERS.slice(0, 2).map(tier => (
                    <div
                      key={tier.key}
                      className={`tier-card ${selectedTier === tier.key ? 'selected' : ''}`}
                      onClick={() => setSelectedTier(tier.key)}
                      style={{ paddingTop: tier.highlight ? 28 : 20 }}
                    >
                      {tier.highlight && (
                        <div className="tier-highlight" style={{ background: tier.highlightColor }}>{tier.highlight}</div>
                      )}
                          ...
                          {sec.items.map(item => (
                            <div key={item.text} className="tier-item">
                              ...
                              <span className="tier-item-text" style={{ color: item.check ? 'var(--stone)' : 'var(--border)' }}>
                                {item.text}
                                {'tag' in item && item.tag && <span className="tier-item-tag">{item.tag}</span>}
                              </span>
                            </div>
                          ))}
                        </div>
                ...
                {/* Bottom row: Annual + Founding */}
                <div className="plan-grid-bottom">
                  {TIERS.slice(2).map(tier => (
                    <div
                      key={tier.key}
                      className={`tier-card ${selectedTier === tier.key ? (tier.key === 'founding' ? 'selected-gold' : 'selected') : ''}`}
                      onClick={() => setSelectedTier(tier.key)}
                      style={{ paddingTop: tier.highlight ? 28 : 20 }}
                    >
                      {tier.highlight && (
                      ...
                      <div className="tier-name">{tier.name}</div>
                      <div style={{ marginBottom: 12 }}>
                        <span className="tier-price" style={{ color: tier.key === 'founding' ? 'var(--amber)' : 'var(--forest)' }}>{tier.price}</span>
                        <span className="tier-period"> {tier.period}</span>
                      </div>
                      <div className="tier-divider" />
                      {tier.sections.map(sec => (
