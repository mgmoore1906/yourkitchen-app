...
type Proposal = {
  ...
  claims: {
    ...
    guest_coordinators: { full_name: string; phone: string | null } | null
    calendar_dates: { date: string; meal_type: string } | null
  } | null
  kitchen_restaurants: { name: string } | null
  menu_items: { name: string; description: string; price: number } | null
}
...
export default function ProposalPage() {
  ...
  useEffect(() => {
    const load = async () => {
      const { data, error } = await supabase
        .from('meal_proposals')
        .select(`
          id, status, coordinator_note,
          claims(
            calendar_date_id,
            guest_coordinators(full_name, phone),
            calendar_dates(date, meal_type)
          ),
  ...
  const mealType   = proposal?.claims?.calendar_dates?.meal_type || 'dinner'
  const coordName  = proposal?.claims?.guest_coordinators?.full_name || 'Someone'
  const date       = proposal?.claims?.calendar_dates?.date
  ...
  if (confirmed) return (
    <Page>
      <div style={{ textAlign: 'center', padding: '16px 0' }}>
        ...
        <p style={subStyle}>The order is placed and DoorDash is on the way. You'll get a tracking link via SMS when a driver is assigned.</p>
        <div style={card}>
          {[
            ['Meal', proposal?.menu_items?.name],
            ['Restaurant', proposal?.kitchen_restaurants?.name],
  ...
  if (declining) return (
    <Page title="Decline proposal">
      ...
      <textarea
        ...
        placeholder={`e.g. "We're actually going out that night — can you try May 29?"`}
    ...
    <Page title="Meal proposal">
      ...
      <div style={card}>
        ...
        <div style={{ display: 'flex', gap: 14, alignItems: 'center', marginBottom: 12 }}>
          <div style={{ width: 48, height: 48, borderRadius: 12, background: S.sageLight, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, flexShrink: 0 }}>
            {mealEmoji}
          </div>
          <div>
            <div style={{ fontFamily: "'Lora', serif", fontSize: 16, fontWeight: 600, color: S.forest }}>
              {proposal?.menu_items?.name}
            </div>
            <div style={{ fontSize: 13, color: S.stone, fontWeight: 300, marginTop: 2 }}>
              {proposal?.menu_items?.description}
            </div>
          </div>
        </div>
        {[
      ...
      <div style={{ background: S.white, border: `0.5px solid ${S.border}`, borderRadius: 12, padding: '13px 16px', marginBottom: 24 }}>
        <p style={{ fontSize: 13, color: S.stone, margin: 0, lineHeight: 1.6 }}>
          📱 Confirming sends the order to DoorDash and notifies {coordName}. Nothing is charged unless you tap <strong>Confirm.</strong>
        </p>
      </div>
