...
// Added: delivery_preference + delivery_note in Step 3
    ...
    <div style={{ background: '#fff', border: '1px solid #DDE8E0', borderRadius: 16, padding: '16px', marginBottom: 20 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 3 }}>
        {cells.map((day, i) => {
          ...
          const isPast  = dateStr < todayStr; const isToday = dateStr === todayStr
          const slots   = dateMap[dateStr] || []; const hasSlots = slots.length > 0
...
function GroupProgress({ groups, currentIndex }: { groups: Group[]; currentIndex: number }) {
  ...
  return (
    <footer style={{ background: '#1E2620', padding: '48px 24px', textAlign: 'center', marginTop: 40, fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap', gap: '4px 0', fontSize: 11, marginBottom: 16 }}>
        {[{ label: 'yourkitchen.app', href: 'https://yourkitchen.app' }, { label: 'marques@yourkitchen.app', href: 'mailto:marques@yourkitchen.app' }, { label: 'Terms', href: '/terms' }, { label: 'Privacy', href: '/privacy' }].map((link, i) => (
          <span key={link.label}>{i > 0 && <span style={{ color: 'rgba(255,255,255,0.25)', margin: '0 8px' }}>·</span>}<a href={link.href} style={{ color: 'rgba(255,255,255,0.3)', textDecoration: 'none' }}>{link.label}</a></span>
        ))}
      </div>
...
const DELIVERY_PREFS = [
  {
    ...
    icon: '🚪',
    title: 'Leave at door',
    subtitle: 'Text me when it arrives — do not knock or ring',
  },
  {
    value: 'hand_to_recipient',
    icon: '🤝',
    title: 'Hand to me',
    subtitle: 'Driver should hand the order directly',
  },
]

function DeliveryPrefSelector({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
    <div style={{ marginBottom: 20 }}>
      <label style={labelStyle}>Delivery preference</label>
      <p style={{ fontSize: 12, color: '#6B7066', fontWeight: 300, marginTop: -4, marginBottom: 10, lineHeight: 1.5 }}>
        For families with a sleeping newborn or someone resting, "leave at door" is often the kinder choice.
      </p>
      <div style={{ display: 'flex', gap: 10 }}>
        {DELIVERY_PREFS.map(pref => (
          <button key={pref.value} onClick={() => onChange(pref.value)}
            style={{ flex: 1, padding: '14px 12px', borderRadius: 12, border: `2px solid ${value === pref.value ? '#3D6B4F' : '#DDE8E0'}`, background: value === pref.value ? '#EAF2ED' : '#fff', cursor: 'pointer', textAlign: 'left', fontFamily: "'DM Sans', sans-serif", transition: 'all 0.15s' }}>
            <div style={{ fontSize: 22, marginBottom: 6 }}>{pref.icon}</div>
            <div style={{ fontSize: 13, fontWeight: 600, color: value === pref.value ? '#3D6B4F' : '#1E2620', marginBottom: 3 }}>{pref.title}</div>
...
export default function CoordKitchenClient({ kitchen, availableDates, restaurants, recentMeals = [] }: any) {
  ...
  const toggleSlot = (slot: any) => setSelectedIds(prev => { const next = new Set(prev); if (next.has(slot.id)) next.delete(slot.id); else next.add(slot.id); return next })
  const removeSlot = (id: string) => setSelectedIds(prev => { const next = new Set(prev); next.delete(id); return next })
  ...
  const filteredRestaurants  = currentGroup ? restaurants.filter((r: any) => (MEAL_TYPE_RESTAURANTS[currentGroup.mealType] || []).includes(r.name)) : []
  const updateGroup          = (index: number, update: Partial<Group>) => setGroups(prev => prev.map((g, i) => i === index ? { ...g, ...update } : g))
  ...
  const handleGroupNext      = () => { if (currentGroupIndex < groups.length - 1) { setCurrentGroupIndex(i => i + 1); setGroupSubStep('restaurant') } else setStep(3) }
  const handleGroupBack      = () => { if (groupSubStep === 'meal') setGroupSubStep('restaurant'); else if (currentGroupIndex > 0) { setCurrentGroupIndex(i => i - 1); setGroupSubStep('meal') } else setStep(1) }
  ...
  const handleSubmit = async () => {
    ...
    const proposals = groups.flatMap(g => g.slots.map(slot => ({
      ...
      <div style={{ minHeight: '100vh', background: '#FAFAF5', fontFamily: "'DM Sans', sans-serif", display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 32px' }}>
          <div style={{ textAlign: 'center', maxWidth: 420 }}>
            ...
            <h1 style={{ fontFamily: "'Lora', serif", fontSize: 28, fontWeight: 500, color: '#1E2620', margin: '0 0 12px' }}>{totalDates > 1 ? `${totalDates} proposals sent!` : 'Proposal sent!'}</h1>
            <p style={{ fontSize: 15, color: '#6B7066', lineHeight: 1.7, margin: '0 0 24px', fontWeight: 300 }}>{recipientFirstName} will get a notification to confirm each one. Thank you. 🧡</p>
            <div style={{ background: '#fff', border: '1px solid #DDE8E0', borderRadius: 16, padding: '20px', textAlign: 'left', marginBottom: 24 }}>
              {groups.map((g, i) => { const mc = MEAL_TYPE_COLORS[g.mealType] || MEAL_TYPE_COLORS.dinner; return (
                <div key={g.mealType} style={{ paddingBottom: i < groups.length - 1 ? 12 : 0, marginBottom: i < groups.length - 1 ? 12 : 0, borderBottom: i < groups.length - 1 ? '1px solid #EAF2ED' : 'none' }}>
                  <span style={{ background: mc.bg, color: mc.color, borderRadius: 20, fontSize: 11, fontWeight: 600, padding: '3px 10px', border: `1px solid ${mc.color}`, display: 'inline-block', marginBottom: 6 }}>{MEAL_TYPE_LABELS[g.mealType]} · {g.slots.length} date{g.slots.length > 1 ? 's' : ''}</span>
            ...
            <div style={{ background: '#1E2620', borderRadius: 16, padding: '20px 24px', textAlign: 'left' }}>
              ...
              <a href="/signup" style={{ display: 'block', background: '#3D6B4F', color: '#fff', borderRadius: 10, padding: '13px', textAlign: 'center', fontSize: 14, fontWeight: 500, textDecoration: 'none', fontFamily: "'DM Sans', sans-serif" }}>Start a free Kitchen →</a>
            </div>
          </div>
        </div>
        <CoordFooter />
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FAFAF5', fontFamily: "'DM Sans', sans-serif", display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '24px', maxWidth: 500, margin: '0 auto', flex: 1, width: '100%' }}>
        ...
        {/* ── Step 2 ── */}
        {step === 2 && currentGroup && (<>
          <GroupProgress groups={groups} currentIndex={currentGroupIndex} />
          {groupSubStep === 'restaurant' && (<>
            <h2 style={h2Style}>{groups.length > 1 ? `Choose a ${currentGroup.mealType} restaurant` : 'Choose a restaurant'}</h2>
            <p style={subStyle}>{groups.length > 1 ? `For your ${MEAL_TYPE_LABELS[currentGroup.mealType].toLowerCase()} date${currentGroup.slots.length > 1 ? 's' : ''} — ${currentGroup.slots.map((s: any) => new Date(s.date + 'T12:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })).join(', ')}` : `${recipientFirstName}'s preferred restaurants — all delivered via DoorDash.`}</p>
              ...
              {filteredRestaurants.map((r: any) => (
                <button key={r.id} onClick={() => handleRestaurantSelect(r)} style={{ background: currentGroup.restaurant?.id === r.id ? '#EAF2ED' : '#fff', border: `2px solid ${currentGroup.restaurant?.id === r.id ? '#3D6B4F' : '#DDE8E0'}`, borderRadius: 16, padding: '14px 16px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 14, fontFamily: "'DM Sans', sans-serif" }}>
                  <div style={{ flex: 1, textAlign: 'left' }}>
                    ...
                    <div style={{ fontSize: 12, color: '#6B7066', fontWeight: 300, marginTop: 2 }}>{r.cuisine} · Delivered</div>
                  </div>
        ...
        {step === 3 && (<>
          ...
          {/* ── DELIVERY PREFERENCE ── */}
          <DeliveryPrefSelector value={deliveryPreference} onChange={v => setDeliveryPreference(v as any)} />

          {/* Optional delivery note */}
          <label style={labelStyle}>Delivery note for driver <span style={{ fontWeight: 300, textTransform: 'none', letterSpacing: 0 }}>(optional)</span></label>
          <input
            value={deliveryNote}
            onChange={e => setDeliveryNote(e.target.value)}
            placeholder="Gate code 4521 · leave at back door"
            style={{ ...inputStyle }}
          />
