        ...
        {cells.map((day, i) => {
          ...
          return (
            <button key={i} onClick={() => { if (!hasSlots || isPast) return; slots.forEach((s: any) => onToggle(s)) }} disabled={isPast || !hasSlots}
              style={{ background: isAnySelected ? '#EAF2ED' : hasSlots ? '#F8FAF8' : 'transparent', border: isToday ? '2px solid #3D6B4F' : isAnySelected ? '2px solid #3D6B4F' : hasSlots ? '1.5px solid #DDE8E0' : '1.5px solid transparent', borderRadius: 10, padding: '8px 2px', minHeight: 52, cursor: hasSlots && !isPast ? 'pointer' : 'default', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 3, fontFamily: "'DM Sans', sans-serif", WebkitTapHighlightColor: 'transparent', opacity: isPast ? 0.35 : 1, transition: 'all 0.1s' }}>
              <span style={{ fontSize: 13, fontWeight: isToday ? 700 : 500, color: isPast ? '#C8D5CA' : isAnySelected ? '#3D6B4F' : '#1E2620', lineHeight: 1 }}>{day}</span>
              {hasSlots && <div style={{ display: 'flex', gap: 2 }}>{slots.map((slot: any, si: number) => { const mc = MEAL_TYPE_COLORS[slot.meal_type] || MEAL_TYPE_COLORS.dinner; return <div key={si} style={{ width: 6, height: 6, borderRadius: '50%', background: selectedIds.has(slot.id) ? mc.color : '#C8D5CA', flexShrink: 0 }} /> })}</div>}
            </button>
          )
        })}
      </div>
...
export default function CoordKitchenClient({ kitchen, availableDates, restaurants }: any) {
  ...
  const handleRestaurantSelect = (restaurant: any) => {
    updateGroup(currentGroupIndex, { restaurant, menuItem: null })
    setGroupSubStep('meal')
  }

  const handleMealSelect = (menuItem: any) => {
    updateGroup(currentGroupIndex, { menuItem })
  }
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#FAFAF5', fontFamily: "'DM Sans', sans-serif", display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ textAlign: 'center', padding: '0 32px', maxWidth: 420 }}>
          ...
          <p style={{ fontSize: 15, color: '#6B7066', lineHeight: 1.7, margin: '0 0 24px', fontWeight: 300 }}>{kitchen.name.split("'")[0]} will get a notification to confirm each one. Thank you. 🧡</p>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #DDE8E0', padding: '20px 24px', textAlign: 'center' }}>
        ...
        {kitchen.dietary_restrictions?.length > 0 && (
          <div style={{ display: 'flex', gap: 6, justifyContent: 'center', flexWrap: 'wrap', marginTop: 10 }}>
            {kitchen.dietary_restrictions.map((d: string) => <span key={d} style={{ background: '#EAF2ED', color: '#3D6B4F', borderRadius: 20, fontSize: 11, fontWeight: 500, padding: '4px 12px' }}>⚠️ {d}</span>)}
          </div>
        )}
      </div>
        ...
        {step === 3 && (
          <>
            ...
            <div style={{ background: '#fff', border: '1px solid #DDE8E0', borderRadius: 12, padding: '14px 16px', marginBottom: 20 }}>
              <p style={{ fontSize: 13, color: '#6B7066', margin: 0, lineHeight: 1.6 }}>💳 You won&apos;t be charged until {kitchen.name.split("'")[0]} confirms each date. No money moves until they say yes.</p>
            </div>
