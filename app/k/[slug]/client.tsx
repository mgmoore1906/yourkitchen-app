    ...
    return (
      <div style={{ minHeight: '100vh', background: '#FAFAF5',
        ...
        <div style={{ textAlign: 'center', padding: '0 32px', maxWidth: 400 }}>
          ...
          <h1 style={{ fontFamily: "'Lora', serif", fontSize: 28, fontWeight: 500,
                       color: '#1E2620', margin: '0 0 12px' }}>Proposal sent!</h1>
          <p style={{ fontSize: 15, color: '#6B7066', lineHeight: 1.7, margin: '0 0 24px', fontWeight: 300 }}>
            {kitchen.name.split("'")[0]} will get a notification to confirm.
            Once they say yes, dinner is on its way. Thank you. 🧡
          </p>
          <div style={{ background: '#fff', border: '1px solid #DDE8E0', borderRadius: 16,
                        padding: '20px', textAlign: 'left', marginBottom: 16 }}>
            {[
              ...
              <div key={l} style={{ display: 'flex', justifyContent: 'space-between',
                                    padding: '9px 0', borderBottom: '1px solid #EAF2ED' }}>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FAFAF5',
      ...
      <div style={{ background: '#1E2620', padding: '20px 24px', textAlign: 'center' }}>
        ...
        <div style={{ fontFamily: "'Lora', serif", fontSize: 24,
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #DDE8E0',
        ...
        <h1 style={{ fontFamily: "'Lora', serif", fontSize: 22, fontWeight: 500,
                     color: '#1E2620', margin: '0 0 4px' }}>{kitchen.name}</h1>
        <p style={{ fontSize: 13, color: '#6B7066', margin: 0, fontWeight: 300 }}>
          {kitchen.address}
        </p>
      ...
      <div style={{ display: 'flex', gap: 6, padding: '20px 24px 0', maxWidth: 500, margin: '0 auto' }}>
        {[1,2,3,4].map(i => (
          <div key={i} style={{ flex: 1, height: 4, borderRadius: 4,
                                background: i <= step ? '#3D6B4F' : '#DDE8E0' }} />
        ))}
      </div>
      ...
      <div style={{ padding: '24px', maxWidth: 500, margin: '0 auto' }}>
        ...
        {step === 1 && (
          <>
            <h2 style={{ fontFamily: "'Lora', serif", fontSize: 22, fontWeight: 500,
                         color: '#1E2620', margin: '0 0 6px' }}>Pick a date</h2>
            <p style={{ fontSize: 14, color: '#6B7066', margin: '0 0 20px', fontWeight: 300 }}>
              These are the dates open on {kitchen.name.split("'")[0]}'s Kitchen.
            </p>
            {availableDates.length === 0 ? (
              <div style={{ background: '#EAF2ED', borderRadius: 14, padding: '20px',
                            textAlign: 'center' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 24 }}>
                {availableDates.map((d: any) => (
                  <button key={d.id} onClick={() => setSelectedDate(d)}
                    style={{ background: selectedDate?.id === d.id ? '#EAF2ED' : '#fff',
                    ...
                    <div>
                      <div style={{ fontFamily: "'Lora', serif", fontSize: 15,
                                    fontWeight: 600, color: '#1E2620' }}>
                        📅 {formatDate(d.date)}
                      </div>
                      <div style={{ fontSize: 12, color: '#6B7066', fontWeight: 300, marginTop: 2 }}>
                        Delivery {d.delivery_window_start?.slice(0,5)} – {d.delivery_window_end?.slice(0,5)}
                      </div>
                    </div>
            ...
            <button onClick={() => setStep(2)} disabled={!selectedDate}
              style={btnStyle(!selectedDate)}>
              Next: Choose Restaurant →
            </button>
          </>
        )}

        {/* Step 2: Pick restaurant */}
        {step === 2 && (
          <>
            <h2 style={{ fontFamily: "'Lora', serif", fontSize: 22, fontWeight: 500,
                         color: '#1E2620', margin: '0 0 6px' }}>Choose a restaurant</h2>
            <p style={{ fontSize: 14, color: '#6B7066', margin: '0 0 20px', fontWeight: 300 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 24 }}>
              {restaurants.map((r: any) => (
                <button key={r.id} onClick={() => setSelectedRestaurant(r)}
                  style={{ background: selectedRestaurant?.id === r.id ? '#EAF2ED' : '#fff',
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={() => setStep(3)} disabled={!selectedRestaurant}
                style={{ ...btnStyle(!selectedRestaurant), flex: 1 }}>
                Next: Select a Meal →
              </button>
            </div>
          </>
        ...
        {step === 3 && (
          <>
            <h2 style={{ fontFamily: "'Lora', serif", fontSize: 22, fontWeight: 500,
                         color: '#1E2620', margin: '0 0 6px' }}>Select a meal</h2>
            <p style={{ fontSize: 14, color: '#6B7066', margin: '0 0 20px', fontWeight: 300 }}>
              ⭐ starred items are {kitchen.name.split("'")[0]}'s favorites.
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 24 }}>
              {selectedRestaurant?.menu_items?.map((item: any) => (
                <button key={item.id} onClick={() => setSelectedItem(item)}
                  style={{ background: selectedItem?.id === item.id ? '#EAF2ED' : '#fff',
                  ...
                  <div style={{ fontSize: 14, fontWeight: 600, color: '#3D6B4F', flexShrink: 0 }}>
                    ${item.price}
                  </div>
                </button>
              ))}
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={() => setStep(4)} disabled={!selectedItem}
                style={{ ...btnStyle(!selectedItem), flex: 1 }}>
                Next: Your Info →
              </button>
            </div>
          </>
        ...
        {/* Step 4: Coordinator info + submit */}
        {step === 4 && (
          <>
            <h2 style={{ fontFamily: "'Lora', serif", fontSize: 22, fontWeight: 500,
                         color: '#1E2620', margin: '0 0 6px' }}>Almost done!</h2>
            <p style={{ fontSize: 14, color: '#6B7066', margin: '0 0 20px', fontWeight: 300 }}>
              Let {kitchen.name.split("'")[0]} know who's sending dinner.
            </p>

            {/* Summary card */}
            <div style={{ background: '#EAF2ED', borderRadius: 14, padding: '16px',
                          marginBottom: 20 }}>
              {[
                ['Meal', selectedItem?.name],
                ['From', selectedRestaurant?.name],
                ...
                <div key={l} style={{ display: 'flex', justifyContent: 'space-between',
                                      padding: '6px 0', borderBottom: '1px solid #C8DDD0' }}>
            ...
            <input value={name} onChange={e => setName(e.target.value)}
              placeholder="Marcus" style={inputStyle} />

            <label style={labelStyle}>Your email</label>
            <input value={email} onChange={e => setEmail(e.target.value)}
              placeholder="marcus@example.com" type="email" style={inputStyle} />

            <label style={labelStyle}>Personal note (optional)</label>
            <textarea value={note} onChange={e => setNote(e.target.value)}
              placeholder="Thinking of you! Hope you enjoy dinner tonight 🧡"
              style={{ ...inputStyle, minHeight: 90, resize: 'none' as const }} />

            <div style={{ background: '#fff', border: '1px solid #DDE8E0', borderRadius: 12,
                          padding: '14px 16px', marginBottom: 20 }}>
              <p style={{ fontSize: 13, color: '#6B7066', margin: 0, lineHeight: 1.6 }}>
                💳 You won't be charged until {kitchen.name.split("'")[0]} confirms.
                No money moves until they say yes.
              </p>
            </div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={handleSubmit} disabled={!name || !email || loading}
                style={{ ...btnStyle(!name || !email), flex: 1 }}>
