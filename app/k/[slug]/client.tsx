...
//   - "Create a free Kitchen" CTA moved from the kitchen header to the
//     BOTTOM of Step 1, after the "Select at least one date" button.
//     Coordinators now see it last, not first.
...
export default function CoordKitchenClient({ kitchen, availableDates, restaurants, recentMeals = [] }: any) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch('/api/proposal', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, email, phone, note, proposals, kitchen_slug: kitchen.slug, tip_amount: tipAmount }) })
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 32px' }}>
          <div style={{ textAlign: 'center', maxWidth: 420 }}>
            ...
            <div style={{ background: '#fff', border: '1px solid #DDE8E0', borderRadius: 16, padding: '20px', textAlign: 'left', marginBottom: 24 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: 12 }}><span style={{ fontSize: 13, color: '#6B7066', fontWeight: 300 }}>Status</span><span style={{ fontSize: 13, color: '#1E2620', fontWeight: 500 }}>⏳ Awaiting confirmation</span></div>
            </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FAFAF5', fontFamily: "'DM Sans', sans-serif", display: 'flex', flexDirection: 'column' }}>
      ...
      {/* Kitchen name + restrictions — CTA removed from here */}
      ...
      <div style={{ padding: '24px', maxWidth: 500, margin: '0 auto', flex: 1, width: '100%' }}>
        ...
        {step === 1 && (<>
          ...
          {/* ── "Create a free Kitchen" CTA — last thing on step 1 ── */}
          <div style={{ marginTop: 24, paddingTop: 24, borderTop: '0.5px solid #DDE8E0', textAlign: 'center' }}>
            <p style={{ fontSize: 12, color: '#6B7066', fontWeight: 300, margin: '0 0 8px', lineHeight: 1.6 }}>
              Does someone you love need their village to show up?
            </p>
            <a href="/signup" style={{ fontSize: 13, color: '#3D6B4F', fontWeight: 500, textDecoration: 'none', fontFamily: "'DM Sans', sans-serif" }}>
