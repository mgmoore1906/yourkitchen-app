...
supabase.from('meal_proposals').select('id,status,delivery_status,meal_type,delivery_date,coordinator_name,restaurant_name,meal_name,tip_amount,stripe_amount,created_at,kitchen_id,proposed_at').gte('created_at', cutoff),
...
const exportXLSX = async (rows: any[], filename: string) => {
if (!rows.length) return
...
const driverFees = confirmed.length * 699 // ~$6.99 avg delivery fee estimate
...
// Meal type breakdown
const mealBreakdown = ['breakfast','lunch','dinner'].map(mt => ({
type: mt,
count: confirmed.filter(p=>p.meal_type===mt).length,
}))
...
// Onboarding time — profile created_at vs kitchen created_at
...
meal: p.meal_name||'',
supporter: p.coordinator_name||'',
order_amount_cents: p.stripe_amount||0,
order_amount_dollars: ((p.stripe_amount||0)/100).toFixed(2),
tip_cents: p.tip_amount||0,
tip_dollars: ((p.tip_amount||0)/100).toFixed(2),
...
address: k.address||'', household_adults: k.household_adults||0,
household_children: k.household_children||0, created_at: k.created_at,
total_orders: confirmed.filter(p=>p.kitchen_id===k.id).length,
...
<Metric label="Gross Merchandise Value" value={fmt$(gmv)} sub={`${confirmed.length} confirmed orders`} color={S.forest}/>
<Metric label="Platform Fee Revenue (3%)" value={fmt$(platformFee)} sub="YourKitchen net" color={S.sage}/>
<Metric label="Tips Paid to Drivers" value={fmt$(tipsTotal)} sub="Passed through 100%" color={S.amber}/>
<Metric label="Est. Driver Fees" value={fmt$(driverFees)} sub={`~$6.99 × ${confirmed.length} orders`} color={S.stone}/>
</div>
...
<div style={{ background:S.white, border:`0.5px solid ${S.border}`, borderRadius:14, padding:'16px 18px' }}>
<p style={{ fontSize:10, fontWeight:700, color:S.stone, letterSpacing:'0.1em', textTransform:'uppercase', margin:'0 0 14px' }}>Kitchen Tiers</p>
{Object.entries(tierCounts).map(([tier, count]) => (
<div key={tier} style={{ display:'flex', alignItems:'center', gap:10, marginBottom:8 }}>
<span style={{ fontSize:12, fontWeight:600, color:S.forest, width:80, flexShrink:0 }}>{tier}</span>
<div style={{ flex:1, height:8, background:S.sageLight, borderRadius:4, overflow:'hidden' }}>
<div style={{ height:'100%', background:tier==='founding'?S.amber:tier==='care'||tier==='annual'?S.sage:S.stone, borderRadius:4, width:`${(count/data.kitchens.length)*100}%` }}/>
</div>
...
<div style={{ background:S.white, border:`0.5px solid ${S.border}`, borderRadius:14, padding:'16px 18px' }}>
<p style={{ fontSize:10, fontWeight:700, color:S.stone, letterSpacing:'0.1em', textTransform:'uppercase', margin:'0 0 14px' }}>Meals by Type</p>
{mealBreakdown.map(({ type, count }) => (
<div key={type} style={{ display:'flex', alignItems:'center', gap:10, marginBottom:8 }}>
<span style={{ fontSize:14 }}>{MEAL_EMOJI[type]}</span>
<span style={{ fontSize:12, fontWeight:600, color:S.forest, width:80, flexShrink:0, textTransform:'capitalize' }}>{type}</span>
<div style={{ flex:1, height:8, background:S.sageLight, borderRadius:4, overflow:'hidden' }}>
<div style={{ height:'100%', background:type==='breakfast'?'#E8834A':type==='lunch'?'#4A8FA8':S.sage, borderRadius:4, width:confirmed.length?`${(count/confirmed.length)*100}%`:'0%' }}/>
</div>
<span style={{ fontSize:12, color:S.stone, width:24, textAlign:'right' }}>{count}</span>
</div>
))}
...
{/* Top restaurants */}
<div style={{ background:S.white, border:`0.5px solid ${S.border}`, borderRadius:14, padding:'16px 18px' }}>
<p style={{ fontSize:10, fontWeight:700, color:S.stone, letterSpacing:'0.1em', textTransform:'uppercase', margin:'0 0 14px' }}>Top Restaurants</p>
...
{/* Top supporters */}
<div style={{ background:S.white, border:`0.5px solid ${S.border}`, borderRadius:14, padding:'16px 18px' }}>
<p style={{ fontSize:10, fontWeight:700, color:S.stone, letterSpacing:'0.1em', textTransform:'uppercase', margin:'0 0 14px' }}>Top Supporters</p>
...
{/* All orders table */}
<div style={{ background:S.white, border:`0.5px solid ${S.border}`, borderRadius:14, padding:'16px 18px', marginTop:8, marginBottom:24 }}>
...
style={{ padding:'6px 14px', borderRadius:8, border:`1.5px solid ${S.border}`, background:S.white, color:S.forest, fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:"'DM Sans',sans-serif" }}>
↓ CSV
</button>
<button onClick={() => exportXLSX(exportRows, `yourkitchen-orders-${range}-${TODAY}`)}
style={{ padding:'6px 14px', borderRadius:8, border:`1.5px solid ${S.sage}`, background:S.sageLight, color:S.sage, fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:"'DM Sans',sans-serif" }}>
↓ Excel
</button>
</div>
</div>
...
<td style={{ padding:'8px 10px', color:S.forest, whiteSpace:'nowrap' }}>{p.stripe_amount?fmt$(p.stripe_amount):'—'}</td>
<td style={{ padding:'8px 10px', color:S.amber, whiteSpace:'nowrap' }}>{p.tip_amount?fmt$(p.tip_amount):'—'}</td>
<td style={{ padding:'8px 10px', whiteSpace:'nowrap' }}>
<span style={{ fontSize:10, fontWeight:700, padding:'2px 8px', borderRadius:20, background:p.status==='confirmed'||p.status==='delivered'?S.sageLight:p.status==='pending'?S.amberLight:S.redLight, color:p.status==='confirmed'||p.status==='delivered'?S.sage:p.status==='pending'?S.amber:S.red }}>
{p.status}
</span>
</td>
</tr>
...
// ── Dispatch tab (existing logic, extracted) ──────────────────────────────────
function DispatchTab({ orders, loading, filter, setFilter, dispatching, cancelling, msgs, cancelReasons, setCancelReasons, handleDispatch, handleCancel, loadOrders, urgentToday }: any) {
...
today: todayOrders.length, tomorrow: tomorrowOrders.length,
week: weekOrders.length, all: allOrders.length,
}
...
const urgentCount = tab.key==='today' ? todayOrders.filter(needsDispatch).length
: tab.key==='tomorrow' ? tomorrowOrders.filter(needsDispatch).length : 0
return (
...
{tab.label}
{count>0&&<span style={{ background:urgentCount>0?S.red:isActive?S.sage:S.border, color:urgentCount>0||isActive?S.white:S.stone, borderRadius:20, padding:'2px 8px', fontSize:11, fontWeight:700 }}>
...
<div style={{ fontSize:13, color:S.stone }}>{order.restaurant_name}{(filter==='week'||filter==='all')&&<span style={{ color:isAwaiting&&order.delivery_date===TODAY?S.red:S.stone }}> · {friendlyDate(order.delivery_date)}</span>}</div>
...
<div><div style={{ fontSize:10, fontWeight:700, color:S.stone, letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:3 }}>Delivery address</div>
<div style={{ fontSize:13, color:S.forest }}>{order.kitchen_name}</div>
<div style={{ fontSize:12, color:S.stone }}>{order.kitchen_address}</div></div>
<div><div style={{ fontSize:10, fontWeight:700, color:S.stone, letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:3 }}>Preference</div>
<div style={{ fontSize:13, color:S.forest }}>{order.delivery_preference==='hand_to_recipient'?'🤝 Hand to recipient':'🚪 Leave at door'}</div>
{order.delivery_note&&<div style={{ fontSize:12, color:S.stone, fontStyle:'italic' }}>"{order.delivery_note}"</div>}</div>
<div><div style={{ fontSize:10, fontWeight:700, color:S.stone, letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:3 }}>Tip</div>
<div style={{ fontSize:13, color:S.forest }}>${((order.tip_amount||0)/100).toFixed(2)}</div></div>
<div><div style={{ fontSize:10, fontWeight:700, color:S.stone, letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:3 }}>Meal</div>
<div style={{ fontSize:13, color:S.forest }}>{order.meal_type?.charAt(0).toUpperCase()+order.meal_type?.slice(1)}</div></div>
</div>
{isAwaiting&&(
...
{view==='dispatch'&&(
<DispatchTab orders={orders} loading={loading} filter={filter} setFilter={setFilter} dispatching={dispatching} cancelling={cancelling} msgs={msgs} cancelReasons={cancelReasons} setCancelReasons={setCancelReasons} handleDispatch={handleDispatch} handleCancel={handleCancel} loadOrders={loadOrders} urgentToday={urgentToday}/>
)}
{view==='analytics'&&<AnalyticsTab adminSecret={adminSecret}/>}
</div>
)
}
