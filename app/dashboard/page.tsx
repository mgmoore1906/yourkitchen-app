...
const S = {
  ...
  amberLight: '#FBF0E4', red: '#B94040', sky: '#4A8FA8',
}
...
type ActiveOrder = { id: string; meal_name: string; restaurant_name: string; coordinator_name: string; doordash_tracking_url: string | null; doordash_delivery_id: string | null; delivery_date: string }
  ...
  return (
    <footer style={{ background: S.forest, padding: '32px 28px 48px', fontFamily: "'DM Sans', sans-serif" }}>
      <div style={{ maxWidth: 680, margin: '0 auto', textAlign: 'center' }}>
        ...
        <div style={{ fontFamily: "'Lora', serif", fontSize: 22, fontWeight: 500, color: S.white, letterSpacing: -0.5, marginBottom: 6 }}>Kitchen</div>
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (profile?.city) setUserAddress(
      ...
      if (k) {
        ...
        // Pending proposals — now using kitchen_id directly
  ...
  return (
    <div style={{ minHeight: '100vh', background: S.cream, fontFamily: "'DM Sans', sans-serif", display: 'flex', flexDirection: 'column' }}>
      ...
      {/* Nav — logo left, tier badge right (hyperlinked to settings) */}
      <nav style={{ background: S.white, borderBottom: `0.5px solid ${S.border}`, padding: '14px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100 }}>
        ...
        {/* Tier badge — hyperlinked to settings plan section */}
        <button
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '28px 24px 48px', flex: 1, width: '100%' }}>
        ...
        {kitchen && (<>
          ...
          {/* ON THE WAY card — active confirmed deliveries (Image 2 style) */}
          {activeOrders.map(order => (
            <div key={order.id} style={{ background: S.white, border: `1.5px solid ${S.sage}`, borderRadius: 18, padding: '18px 20px', marginBottom: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 14 }}>
                <span style={{ fontSize: 14 }}>🚗</span>
                <span style={{ fontSize: 10, fontWeight: 700, color: S.sage, letterSpacing: '0.1em', textTransform: 'uppercase' }}>On the Way</span>
              </div>
              <div style={{ display: 'flex', gap: 14, alignItems: 'center', marginBottom: 14 }}>
                <div style={{ width: 48, height: 48, borderRadius: 14, background: S.sageLight, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <span style={{ fontSize: 22 }}>🧡</span>
                </div>
                <div>
                  <div style={{ fontFamily: "'Lora', serif", fontSize: 16, fontWeight: 600, color: S.forest }}>{order.meal_name}</div>
                  <div style={{ fontSize: 13, color: S.stone, fontWeight: 300 }}>from {order.restaurant_name}{order.coordinator_name ? ` · sent by ${order.coordinator_name}` : ''}</div>
                </div>
              </div>
              {[
                ['Date', order.delivery_date ? new Date(order.delivery_date + 'T12:00:00').toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' }) : '—'],
                order.doordash_delivery_id ? ['Delivery ID', order.doordash_delivery_id] : null,
              ].filter(Boolean).map(([l, v]) => (
                <div key={l} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: `0.5px solid ${S.sageLight}` }}>
                  <span style={{ fontSize: 13, color: S.stone, fontWeight: 300 }}>{l}</span>
                  <span style={{ fontSize: 12, color: S.forest, fontWeight: 500, fontFamily: order.doordash_delivery_id && l === 'Delivery ID' ? 'monospace' : "'DM Sans', sans-serif", textAlign: 'right', maxWidth: '65%', wordBreak: 'break-all' }}>{v}</span>
                </div>
              ))}
              {order.doordash_tracking_url && (
                <a href={order.doordash_tracking_url} target="_blank" rel="noopener noreferrer"
                  style={{ display: 'block', background: S.forest, color: S.white, borderRadius: 11, padding: '13px', fontSize: 14, fontWeight: 600, textDecoration: 'none', textAlign: 'center', marginTop: 14, fontFamily: "'DM Sans', sans-serif" }}>
                  🚗 Track My Delivery
                </a>
              )}
            </div>
          ...
          {proposals.map(p => (
            <div key={p.id} style={{ background: S.forest, borderRadius: 18, padding: '18px 20px', marginBottom: 16 }}>
              ...
              <div style={{ fontFamily: "'Lora', serif", fontSize: 17, color: S.white, fontWeight: 500, margin: '0 0 4px' }}>
                {p.coordinator_name} wants to send you {p.meal_type || 'dinner'}
              </div>
              <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.6)', marginBottom: 14, fontWeight: 300 }}>
                ...
                {p.delivery_date && ` · ${new Date(p.delivery_date + 'T12:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}`}
              </div>
              <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', margin: '0 0 14px', fontWeight: 300 }}>
          ...
          {/* Kitchen name + status strip (Image 2 bottom section) */}
          {activeOrders.length === 0 && proposals.length === 0 && (
            <div style={{ marginBottom: 20 }}>
              <h2 style={{ fontFamily: "'Lora', serif", fontSize: 22, fontWeight: 500, color: S.forest, margin: '0 0 4px' }}>
                {kitchen.name} 👋
              </h2>
              <p style={{ fontSize: 14, color: S.stone, fontWeight: 300, margin: 0 }}>
          ...
          {/* Quick Actions — 4 tiles, sign out as 4th */}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 16 }}>
            {[
              ...
              { icon: '🔓', label: 'Sign Out',        action: signOut,                                   badge: null },
            ].map(a => (
              <button key={a.label} onClick={a.action}
                style={{ background: S.white, border: `0.5px solid ${S.border}`, borderRadius: 14, padding: '16px', display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", textAlign: 'left' }}>
          ...
          {/* Pending proposals badge row — shows count if any */}
          {proposals.length > 0 && (
            <button onClick={() => proposals.length === 1 ? router.push(`/proposals/${proposals[0].id}`) : router.push('/kitchen/orders')}
              style={{ width: '100%', background: S.amberLight, border: `1.5px solid ${S.amber}`, borderRadius: 14, padding: '14px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", marginBottom: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 28, height: 28, borderRadius: '50%', background: S.amber, display: 'flex', alignItems: 'center', justifyContent: 'center', color: S.white, fontSize: 12, fontWeight: 700, flexShrink: 0 }}>
                  {proposals.length}
                </div>
                <span style={{ fontSize: 13, fontWeight: 600, color: S.forest }}>
                  {proposals.length === 1 ? '1 meal proposal waiting for your reply' : `${proposals.length} meal proposals waiting for your reply`}
                </span>
              </div>
              <span style={{ color: S.amber, fontSize: 18 }}>›</span>
            </button>
          )}

          {/* Account summary — Image 1 design */}
          <div style={{ background: S.white, border: `0.5px solid ${S.border}`, borderRadius: 16, overflow: 'hidden' }}>
            {/* 2×2 grid */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0 }}>
              {[
                { label: 'STATUS', val: <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><span>✅</span> Active</span> },
                { label: 'TIER',   val: <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>{tier.star && <span style={{ color: tier.color }}>★</span>} {tier.badge}</span> },
                { label: 'ADDRESS',   val: userAddress || '—' },
                { label: 'HOUSEHOLD', val: householdSize ? `${householdSize} people` : '—' },
              ].map((card, i) => (
                <div key={card.label} style={{
                  padding: '16px 18px',
                  borderRight: i % 2 === 0 ? `0.5px solid ${S.border}` : 'none',
                  borderBottom: i < 2 ? `0.5px solid ${S.border}` : 'none',
                }}>
            ...
            {/* Tier badge card — amber for founding, sage for others */}
            <div style={{
              margin: '0 14px 14px',
              background: tier.bg,
              ...
              <div>
                <p style={{ fontSize: 10, fontWeight: 700, color: tier.color, letterSpacing: '0.1em', textTransform: 'uppercase', margin: '0 0 4px' }}>
                  {tier.badge}
                </p>
                <p style={{ fontSize: 13, color: S.stone, fontWeight: 300, margin: 0, lineHeight: 1.5 }}>{tier.desc}</p>
              </div>
              {tier.star && <span style={{ fontSize: 28, marginLeft: 12, flexShrink: 0 }}>✦</span>}
            </div>
          </div>
