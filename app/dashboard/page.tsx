...
function Footer() {
  return (
    <footer style={{ background: '#1E2620', padding: '32px 28px 48px', fontFamily: "'DM Sans', sans-serif" }}>
      <div style={{ maxWidth: 680, margin: '0 auto', textAlign: 'center' }}>
        ...
        <div style={{ fontFamily: "'Lora', serif", fontSize: 22, fontWeight: 500, color: '#fff', marginBottom: 6 }}>Kitchen</div>
        <p style={{ fontFamily: "'Lora', serif", fontStyle: 'italic', fontSize: 13, color: '#6B9E7E', margin: '0 0 18px' }}>"Your kitchen, covered."</p>
    ...
    <div style={{ minHeight: '100vh', background: S.cream, fontFamily: "'DM Sans', sans-serif", display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '28px 24px 48px', flex: 1, width: '100%' }}>
        ...
        {kitchen && (<>
          ...
          {/* ON THE WAY — active confirmed deliveries */}
            ...
            <div key={order.id} style={{ background: S.white, border: `1.5px solid ${S.sage}`, borderRadius: 18, padding: '18px 20px', marginBottom: 16 }}>
              ...
              <div style={{ display: 'flex', gap: 14, alignItems: 'center', marginBottom: 14 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 13, color: S.stone, fontWeight: 300 }}>
                    from {order.restaurant_name}{order.coordinator_name ? ` · sent by ${order.coordinator_name}` : ''}
                  </div>
                </div>
              </div>

              {/* Detail rows — typed as [string, string][] to satisfy TS */}
              {(
                [
                  ['Date', order.delivery_date
                    ? new Date(order.delivery_date + 'T12:00:00').toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })
                    : '—'],
                  ...(order.doordash_delivery_id ? [['Delivery ID', order.doordash_delivery_id]] : []),
                ] as [string, string][]
              ).map(([l, v]) => (
                <div key={l} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: `0.5px solid ${S.sageLight}` }}>
                  ...
                  <span style={{ fontSize: 12, color: S.forest, fontWeight: 500, textAlign: 'right', maxWidth: '65%', wordBreak: 'break-all' }}>{v}</span>
                </div>
              ))}
          ...
          {proposals.map(p => (
            <div key={p.id} style={{ background: S.forest, borderRadius: 18, padding: '18px 20px', marginBottom: 16 }}>
              ...
              <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', margin: '0 0 14px', fontWeight: 300 }}>No charge until you confirm.</p>
          ...
          {/* Kitchen name strip when nothing pending */}
          ...
          {proposals.length > 0 && (
            <button
              onClick={() => proposals.length === 1 ? router.push(`/proposals/${proposals[0].id}`) : router.push('/kitchen/orders')}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <span style={{ fontSize: 13, fontWeight: 600, color: S.forest }}>
                  {proposals.length === 1 ? '1 meal proposal waiting for your reply' : `${proposals.length} meal proposals waiting`}
                </span>
              </div>
