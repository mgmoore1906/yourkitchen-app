...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (k) {
        setKitchen(k)
        // Build URL from current domain — works on Vercel, custom domain, localhost
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '28px 24px 80px' }}>
        ...
        {kitchen && (<>
          ...
          <div style={{ background: S.white, border: `0.5px dashed ${S.sageMid}`, borderRadius: 16, padding: '16px 20px', marginBottom: 20 }}>
            <p style={{ fontSize: 10, fontWeight: 700, color: S.stone, letterSpacing: '0.08em', textTransform: 'uppercase', margin: '0 0 6px' }}>
              Your Kitchen Link
            </p>
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      fontFamily: "'DM Sans', sans-serif", opacity: isPast ? 0.35 : 1, transition: 'all 0.1s',
                    }}
                  >
                    ...
                    {slots.length > 0 ? (
                      <div style={{ display: 'flex', gap: 2, flexWrap: 'wrap', justifyContent: 'center' }}>
                        {slots.map((s, si) => (
                          <div key={si} style={{ width: 6, height: 6, borderRadius: '50%', background: MEAL_COLORS[s.meal_type]?.color || S.sage }} />
                        ))}
                      </div>
                    ) : !isPast ? (
          ...
          {/* ── Add-date panel (appears below calendar when a date is clicked) ── */}
          {selectedDate && (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
                {Object.entries(MEAL_COLORS).map(([type, mc]) => {
                  ...
                  return (
                    <button
                      key={type}
                      onClick={() => !alreadyHas && !adding && handleAddSlot(type)}
                      disabled={alreadyHas || adding}
                      style={{ background: alreadyHas ? '#F5F5F5' : mc.bg, border: `1.5px solid ${alreadyHas ? '#DDD' : mc.color}`, borderRadius: 10, padding: '12px 8px', cursor: alreadyHas || adding ? 'default' : 'pointer', opacity: alreadyHas ? 0.5 : 1, fontFamily: "'DM Sans', sans-serif" }}
                    >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            {[
              ...
              { icon: '🔗', label: 'Copy Kitchen Link', action: copyLink },
            ].map(a => (
              <button
                key={a.label}
                onClick={a.action}
